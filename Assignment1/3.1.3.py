...
def main():
    ...
    x_temp = x.copy()
    t_temp = t.copy()
    x_temp, t_temp = dg.generate_training_a_b(x_temp, t_temp, 0.25)
    ...
    x_temp = x.copy()
    t_temp = t.copy()
    x_temp, t_temp = dg.generate_training_a(x_temp, t_temp, 0.5)
    x_grid = np.arange(min(x_temp[0,:]), max(x_temp[0,:]), (max(x_temp[0,:]) - min(x_temp[0,:])) / SAMPLES)
    # w can be equal, since t.shape[0] and x.shape[0] will not change (only column removal)
    ...
    x_temp = x.copy()
    t_temp = t.copy()
    x_temp, t_temp = dg.generate_training_b(x_temp, t_temp, 0.5)
    x_grid = np.arange(min(x_temp[0,:]), max(x_temp[0,:]), (max(x_temp[0,:]) - min(x_temp[0,:])) / SAMPLES)
    # w can be equal, since t.shape[0] and x.shape[0] will not change (only column removal)
    ...
    x_temp = x.copy()
    t_temp = t.copy()
    x_temp, t_temp = dg.generate_training_a_subsets(x_temp, t_temp, 0.2, 0.8)
    x_grid = np.arange(min(x_temp[0,:]), max(x_temp[0,:]), (max(x_temp[0,:]) - min(x_temp[0,:])) / SAMPLES)
    # w can be equal, since t.shape[0] and x.shape[0] will not change (only column removal)
